...
username = f'{user}@edinburghairport.com'
...
start_of_two_weeks_ago = start_of_this_week - pd.Timedelta(weeks=2)
start_of_three_weeks_ago = start_of_this_week - pd.Timedelta(weeks=3)

# ✅ This is your target window:
start_date = start_of_three_weeks_ago          # 3 weeks ago (Monday)
end_date = start_of_last_week - pd.Timedelta(days=1)   # end of 2 weeks ago (Sunday



# Pull EAL.FlightPerformance for the last two weeks, not including this week
sql = f"""
SELECT *
FROM EAL.FlightPerformance
WHERE [ActualDateTime_Local] >= '{start_date.strftime('%Y-%m-%d')}'
    AND [ActualDateTime_Local] <= '{end_date.strftime('%Y-%m-%d')}'
"""
...
WHERE [CheckInStarted] >= '{start_date.strftime('%Y-%m-%d')}'
    AND [CheckInStarted] <= '{end_date.strftime('%Y-%m-%d')}'
"""
...
SELECT *
FROM FastPark.v_EntryAndExits
WHERE [ActualCheckedOutDate] >= '{start_date.strftime('%Y-%m-%d')}'
    AND [ActualCheckedOutDate] <= '{end_date.strftime('%Y-%m-%d')}'
"""
...
# %%
# Filter for FastPark bookings with status 'B' (booked) and group by entryDate and exitDate to get daily counts
...
# %%
df_entries = fastpark_actual_entries_df.drop_duplicates(subset='BookingReference').copy()
...
# filter for departure flights and passenger flights, then group by actual date to get total passengers per day for departures
...
# Calculate penetration rate by weekday for entries
...
# %%
# Repeate the same analysis for exits

df_exits = fastpark_actual_exits_df.drop_duplicates(subset='BookingReference').copy()
...
# %%
# Filter for future departure flights and passenger flights
...
# Create a DataFrame to store the forecasted entries along with the corresponding dates and penetration rates
...
# %%
# Filter for future arrival flights and passenger flights
...
merged_df = merged_df[merged_df.index.date != today.date()].copy()
...
# % increase for each day of next 2 weeks
...
start_date = pd.Timestamp(year=today.year, month=6, day=1)
end_date = pd.Timestamp(year=start_date.year, month=6, day=30)
...
# easter_first_mask = merged_df['date'].between(pd.Timestamp('2026-04-11'), pd.Timestamp('2026-04-13'), inclusive='both')
# easter_second_mask = merged_df['date'].between(pd.Timestamp('2026-04-16'), pd.Timestamp('2026-04-20'), inclusive='both')
# april_mask = merged_df['date'].dt.month == 4
# rest_of_april_mask = april_mask & ~easter_first_mask &~easter_second_mask

# Apply different percentage increases based on the day of the week
# mtw = [0,1,2]
# twt = [1,2,3]
# tfss = [3,4,5,6]
# mtw_mask = merged_df['date'].dt.dayofweek.isin(mtw)
# tfss_mask = merged_df['date'].dt.dayofweek.isin(tfss)
# twt_mask = merged_df['date'].dt.dayofweek.isin(twt)


print(merged_df.head(14))

merged_df.loc[june_mask, ['entry_forecast']] *= 1.00
merged_df.loc[june_mask, ['exit_forecast']] *= 1.00

# merged_df.loc[may_mask & mtw_mask, ['entry_forecast', 'exit_forecast']] *= 1
# merged_df.loc[may_mask & tfss_mask, ['entry_forecast', 'exit_forecast']] *= 1
...
# merged_df.to_csv(r'C:\Users\{}\Edinburgh Airport Limited\Shared Files - Business Planning\Haoran\FastPark\FastPark Model Rebuild 2025\FastPark Short-term Model Rebuild\Daily Forecast Output\{}_daily_forecast.csv'.format(user, today_str))
...
for i, weekday in enumerate(weekday_entry_names):
    ...
    entries_weekday = df_entries[df_entries['weekday'] == i].copy()
...
# Iterate over each date in forecasted_entries_df
...
for i, weekday in enumerate(weekday_exit_names):
    ...
    exits_weekday = df_exits[df_exits['weekday'] == i].copy()
...
# if user == "haoran_hong":
#     csv_path = r"C:\Users\haoran_hong\Edinburgh Airport Limited\Shared Files - Business Planning\Haoran\FastPark\FastPark Model Rebuild 2025\FastPark Short-term Model Rebuild\FastPark Hourly Forecast Output.csv"

# elif user == "christina_k":
#     csv_path = r"C:\Users\Christina_K\Edinburgh Airport Limited\Shared Files - Business Planning\Haoran\FastPark\FastPark Model Rebuild 2025\FastPark Short-term Model Rebuild\FastPark Hourly Forecast Output.csv"

# elif user == "jamie_douglas":
#     csv_path = r'C:\Users\jamie_douglas\Edinburgh Airport Limited\Shared Files - Business Planning\Haoran\FastPark\FastPark Model Rebuild 2025\FastPark Short-term Model Rebuild\FastPark Hourly Forecast Output.csv'.format(user)
# elif user == "aryan_puri":
#     csv_path = r'C:\Users\aryan_puri\Edinburgh Airport Limited\Shared Files - Business Planning\Haoran\FastPark\FastPark Model Rebuild 2025\FastPark Short-term Model Rebuild\FastPark Hourly Forecast Output.csv'.format(user)
# exisiting_df = pd.read_csv(csv_path, index_col=0)

# exisiting_df['date'] = pd.to_datetime(exisiting_df['date']).dt.date
...
# # exisiting_df_copy['entry_forecast'] = np.ceil(exisiting_df_copy['entry_forecast']).astype(float)
# # exisiting_df_copy['exit_forecast'] = np.ceil(exisiting_df_copy['exit_forecast']).astype(float)
...
# # save file to folder and to DWH path for upload
...
# exisiting_df_copy.to_csv(r'C:\Users\{}\Edinburgh Airport Limited\Shared Files - Business Planning\Haoran\FastPark\FastPark Model Rebuild 2025\FastPark Short-term Model Rebuild\Daily Forecast Output\{}_daily_forecast_DWH.csv'.format(user,today_str), index=False)
...
# # %%
# # save to workforce folder for daily plan pull
# if user == "haoran_hong":
#     # Save combined_hourly_forecast_df to the additional directory
#     additional_dir = r'C:\Users\{}\Edinburgh Airport Limited\Workforce Management - Documents\EASL\FastPark\FastPark Forecast'.format(user)
#     exisiting_df.to_csv(f'{additional_dir}\\FastPark Hourly Forecast Output.csv')
# elif user == "christina_k":
#     # Save combined_hourly_forecast_df to the additional directory
#     additional_dir = r'C:\Users\Christina_K\Edinburgh Airport Limited\Workforce Management - FastPark Forecast'
#     exisiting_df.to_csv(f'{additional_dir}\\FastPark Hourly Forecast Output.csv')
# elif user == "jamie_douglas":
#     additional_dir = r'C:\Users\jamie_douglas\Edinburgh Airport Limited\Workforce Management - FastPark Forecast'.format(user)
#     exisiting_df.to_csv(f'{additional_dir}\\FastPark Hourly Forecast Output.csv')
# elif user == "aryan_puri":
#     additional_dir = r'C:\Users\aryan_puri\Edinburgh Airport Limited\Workforce Management - FastPark Forecast'.format(user)
#     exisiting_df.to_csv(f'{additional_dir}\\FastPark Hourly Forecast Output.csv')




# print("PASSED WORKFORCE SAVE")

# if today.weekday() == 0:
#     print("ENTERED WEEKLY BLOCK")
...
# print("STARTING ENTRY HOURLY CSV")
...
# print("STARTING EXIT HOURLY CSV")
...
# # Define the path to weekly excel tracker

# if user == "haoran_hong":
#     excel_path = "FastPark Forecast Accuracy Tracker NEW.xlsx"
# elif user == "christina_k":
#     excel_path = r"c:\Users\Christina_K\Edinburgh Airport Limited\Shared Files - Business Planning\Haoran\FastPark\FastPark Model Rebuild 2025\FastPark Short-term Model Rebuild\FastPark Forecast Accuracy Tracker NEW.xlsx"

# elif user == "jamie_douglas":
#     excel_path = r'C:\Users\jamie_douglas\Edinburgh Airport Limited\Shared Files - Business Planning\Haoran\FastPark\FastPark Model Rebuild 2025\FastPark Short-term Model Rebuild\FastPark Forecast Accuracy Tracker NEW.xlsx'
# elif user == "aryan_puri":
#     excel_path = r'C:\Users\aryan_puri\Edinburgh Airport Limited\Shared Files - Business Planning\Haoran\FastPark\FastPark Model Rebuild 2025\FastPark Short-term Model Rebuild\FastPark Forecast Accuracy Tracker NEW.xlsx'
...
# # Find the starting row to write (match first_exit_date)
...
# # Skip overwriting for dates, currently not used
